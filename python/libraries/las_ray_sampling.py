...
# resample voxel space with resampling config files:
    ...
    # las_ray_sample_hemi_optimization.py -- sample over hemispherical set of rays over points corresponding to photography
    # las_ray_sample_img_from_pts.py (STILL HAS BUGS) -- sample over rays using mercator projection of sphere around given points
...
def save_vox_meta(vox):
    """
    initiate voxel metadata hdf5 file
    :param vox: voxel object
    :return: None
    """
...
def save_vox_post(vox):
    """
    save voxel posterior data object to hdf5 file
    :param vox: voxel object
    :return: None
    """
    with h5py.File(vox.vox_hdf5, 'a') as h5f:
...
def load_vox(hdf5_path, load_data=False, load_post=False, load_post_data=False):
    """
    load data/metadata from vox hdf5 file
    :param hdf5_path: file to load
    :param load_data: boolean: load count data?
    :param load_post: boolean: load posterior metadata?
    :param load_post_data: boolean: load posterior data?
    :return: voxel object
    """
    vox = VoxelObj()
...
def duplicate_vox(vox, hdf5_out, duplicate_post=False, z_slices=1):
    """
    deep copy of voxel object
    :param vox: voxel object
    :param hdf5_out: hdf5 file for new copy
    :param duplicate_post: boolean: copy posterior data?
    :param z_slices: z-slices for memory management (more slices if crashes when copying)
    :return: None
    """
...
def z_rotmat(theta):
    """
    3d rotation matrix about vertical axis, clockwise by theta (radians)
    :param theta: angle of rotation in radians, clockwise (looking down)
    :return:
    """
    if theta is None:
        theta = 0
...
def utm_to_vox(vox, utm_points, rotation=True):
    """
    transform from UTM to voxel space coordinates
    :param vox: voxel object
    :param utm_points: df of UTM coordinates
    :param rotation: UTM rotation binary
    :return: df of voxel space coordinates
    """
    if rotation:
...
def vox_to_utm(vox, vox_points, rotation=True):
    """
    transform from voxel space to UTM coordinates
    :param vox: voxel object
    :param vox_points: df of voxel space coordinates
    :param rotation: UTM rotation binary
    :return: df of UTM coordinates
    """
    if rotation:
        theta = vox.cw_rotation
    else:
...
def add_points_to_voxels(vox, dataset, points):
    """
    voxel binning of points by coordinates
    :param vox: voxel object
    :param dataset: which voxel dataset to add points to
    :param points: coordinates of points to be added
    :return: voxel object with added points
    """

    # convert UTM to voxel coordinate system
...
def interpolate_to_bounding_box(fixed_points, flex_points, bb=None, cw_rotation=0):
    """
    Clip rays between fixed_points and flex_points by bounding box bb, or by bounding box of fixed points if bb is None
    :param fixed_points: dataframe of ray endpoint coordinates, fixed and will be returned in function output
    :param flex_points: dataframe of ray endpoint coordinates, to be replaced with ray intercept with bounding box (if exists)
    :param bb: explicit bounding box to clip rays by [[x_lower, y_lower, z_lower], [x_upper, y_upper, z_upper]]. If
    none, outermose bounding box of fixed_points is used (considering rotation about vertical axis if specified)
    :param cw_rotation: rotation of bounding box about vertical axis in clockwise (looking down) radians
    :return: bb_points: dataframe of ray endpoints bounded by bounding box, to use in place of flex_points
    """
    ...
    if bb:
        lb = bb[0] # lower bounds
        ub = bb[1] # upper bounds
        if np.any((lb > fixed_points) | (fixed_points > ub)):
            raise Exception('not all fixed points are contained within provided bounding box. Process aborted.')
    else:
        # define bounding box by that of fixed points
        lb = np.min(fixed_points, axis=0) # lower bound
        ub = np.max(fixed_points, axis=0) # upper bound

    # for each dimension
    for ii in range(0, 3):
        # for flex_points points outside bounding box
        lows = (bb_points[:, ii] < lb[ii])
        highs = (bb_points[:, ii] > ub[ii])
        for jj in range(0, 3):
            # interpolate flex_points to bounding box
...
def interpolate_to_z_slice(p1, p0, z_low_utm, z_high_utm):
    """
    calculate segment of rays within elevation slice from z_low to z_high (m AMSL)
    :param p1: 1st set ray endpoints
    :param p0: 2nd set ray endpoints
    :param z_low_utm: lower elevation bound of z_slice layer
    :param z_high_utm: upper elevation bound of z_slice layer
    :return: z1 (interpolated p1 points), z0 (interpolated p0 points)
    """
...
def las_ray_sample_by_z_slice(vox, z_slices=1, fail_overflow=False):
    """
    generates voxel samples along rays made by lidar sensor coords and corresponding first returns
    :param vox: voxel object
    :param z_slices: number of elevation layers (use more if memory issues encountered)
    :param fail_overflow: boolean: do you want to run checks to make sure voxel counts do not overflow? This is very computationally expensive.
    :return: voxel object
    """
...
def vox_addition(voxList, vox_out, z_slices=1):
    """
    add voxel space count datasets (samples and returns)
    :param voxList: list of hdf5 voxel files
    :param vox_out: path to new hdf5 file (to be created)
    :param z_slices: number of elevation slices for memory management (use more if memory issues occur)
    :return:
    """
...
def beta(rays, path_returns, path_samples, vox_sample_length, agg_sample_length, prior, weights=1):
    """
    calculates returns along ray by sum of calculated beta posterior hyerparameters
    :param rays: rays over which to sample beta hyperparameters
    :param path_returns: returns along ray
    :param path_samples: samples along ray
    :param vox_sample_length: sample length used in voxel space sampling
    :param agg_sample_length: sample length used in resampling
    :param prior: beta prior parameters
    :param weights: unused
    :return: rays df with return estimates
    """
...
def beta_lookup(rays, post_a, post_b, weights=1):
    """
    calculates returns along ray by sum of lookup of beta posterior hyerparameters
    :param rays: rays df
    :param post_a: alpha hyperparameter values along rays
    :param post_b: beta hyperparameter valus along rays
    :param weights: unused
    :return: rays df with return estimates
    """
...
def single_ray_agg(vox, rays, agg_sample_length, lookup_db="posterior"):
    """
    point (re)sampling of voxel space database along rays of interest for aggregation of modeled returns along rays
    :param vox: voxel object
    :param rays: rays of interest
    :param agg_sample_length: sample length for resampling
    :param lookup_db: which database in vox to sample: "posterior" or "count"?
    :return: rays_out df of results
    """
...
def vox_agg(origin, vox_sub, img_size, max_phi=np.pi/2, max_dist=50, min_dist=0, ref="edge"):
    """
    UNFINISHED -- volumetric aggregation of voxel space along cones of interest, for (ideally) more efficient
    calculation of large-angle return/transmittance metrics.
    :param origin: point of cone origin
    :param vox_sub: voxel object
    :param img_size: number of phi samples along x/y axes
    :param max_phi: cone width
    :param max_dist: cone length
    :param min_dist: minimum distance from origin to consider
    :param ref: use "center" or "edge" of voxels?
    :return:
    """
...
def agg_ray_chunk(chunksize, vox, rays, agg_method, agg_sample_length, lookup_db='posterior'):
    """

    :param chunksize: split rays into chunks for memory management
    :param vox: voxel object
    :param rays: rays of interest
    :param agg_method: currently only supports "single_ray_agg". "vox_agg" needs more development
    :param agg_sample_length:
    :param lookup_db: which database in voxel object to use: "posterior" or "count"
    :return: rays_out with samples
    """
    n_rays = len(rays)
...
def dem_to_points(dem_in, mask_in=None):
    """
    gridded sampling from DEM points
    :param dem_in: dem file
    :param mask_in: binary mask of DEM region of interest
    :return: dem raster points
    """
    if mask_in is None:
        ddict = {'z_m': dem_in}
    else:
    ...
    # load data
    pts = raslib.pd_sample_raster_gdal(ddict)
...
def points_to_angle_rays(pts, vox, phi, theta, min_dist=0, max_dist=100):
    """
    calculate rays of interest from specified angles
...
def ray_stats_to_dem(rays, dem_in, file_out):
    """
    gridded ray sampling output save to raster like DEM
    :param rays: ray sampling output
    :param dem_in: dem template file
    :param file_out: results file out
    :return:
    """
    # load raster dem
    ras = raslib.raster_load(dem_in)
    ...
    raslib.raster_save(ras, file_out)
...
def hemi_vectors(img_size, max_phi, ref="center"):
    """
    identify vecotrs within the hemisphere, relative to origin
    :param img_size: number of phi samples along x/y axes
    :param max_phi: maximum value of phi considered
    :param ref: "center" or "edge" of voxel?
    :return: rays of interest, relative to origin
    """
...
def point_to_hemi_rays(origin, vox, img_size, max_phi=np.pi/2, max_dist=50, min_dist=0):
    """
    calculate rays of interest for a given point
    :param origin: point of interest
    :param vox: voxel object (for bounding box)
    :param img_size: number of phi values along x/y axes
    :param max_phi: maximum phi value considered
    :param max_dist: maximum distance from origin considered
    :param min_dist: minimum distance from origin considered
    :return: rays
    """
...
def hemi_rays_to_img(rays_out, img_path, img_size, area_factor):
    """
    export hemi output as image (UNUSED?)
    :param rays_out: rays with return estimates
    :param img_path: output image file
    :param img_size: number of samples along x/y axes
...
def las_to_vox(vox, z_slices, run_las_traj=True, fail_overflow=False, posterior_calc=True):
    """
    calculate voxel space counts of returns and samples from return point cloud and trajectory files
    :param vox: voxel object
    :param z_slices: number of elevation slices for memory management (increase if memory issues occur)
    :param run_las_traj: boolean: do you want to create new hdf5 file of merged return and trajectory points?
    :param fail_overflow: boolean: do you want to check for overflow error in counting samples/returns? Very computationally expensive...
    :param posterior_calc: boolean: do you want to calculate a posterior dataset from count observations?
    :return: voxel object
    """
    if run_las_traj:
...
def vox_to_las(vox, las_out, samps_per_vox=1, sample_threshold=0):
    """
    generate a resampled point cloud according to a given number of samples per voxel and each voxel mean return rate.
    Subvoxel noise added to point coordinates.
    :param vox: voxel object
    :param las_out: las_file to be generated
    :param samps_per_vox: Number of samples to be drawn
    :param sample_threshold: Only sample voxels with corresponding sample counts greater than this number (for handling
    unobserved regions, thresholding noise, if desired)
    :return:
    """
    # generate las file with point density according to return rate by resampling ("resampled point cloud")
    ...
    valid = (vox.sample_data >= sample_threshold)  # do not try to sample voxels with samples below the sample threshold
...
def subset_vox(pts, vox, buffer, lookup_db='posterior'):
    """
    subset voxel object to area of interest for memory management
    :param pts: points of interest
    :param vox: original voxel object
    :param buffer: greatest buffer around points of interest to consider
    :param lookup_db: which database to copy over: "posterior" or "count"
    :return: subset voxel object
    """
...
class RaySampleHemiMetaObj(object):
    # define ray sample hemi metadata class
    def __init__(self):
        # preload metadata
        self.id = None
        self.file_name = None
...
class RaySampleGridMetaObj(object):
    # define ray sample grid metadata class
    def __init__(self):
        # preload metadata
        self.id = None
        self.file_name = None
...
def rs_hemigen(rshmeta, vox, tile_count_1d=1, n_cores=1, initial_index=0):
    """
    ray sampling over hemispherical rays initialization function
    :param rshmeta: ray sampling hemi metadata object
    :param vox: voxel object
    :param tile_count_1d: 2d tiling id
    :param n_cores: number of processing cores
    :param initial_index: which point to begin at?
    :return:
    """
...
def rshm_iterate(rshm, rshmeta, vox, log_path, process_id=0, nrows=4):
    """
    ray sampling over hemispheres, parallel function
    :param rshm: dataframe of points for which to calculate hemispheres
    :param rshmeta: ray sampling over hemispheres metadata object
    :param vox: voxel object
    :param log_path: log file path
    :param process_id: process id for parallelism
    :param nrows: number of reporting rows (REPORTING is BUGGY!... but the end product works regardless)
    :return: rshm
    """
...
def rs_gridgen(rsgmeta, vox, chunksize=1000000, initial_index=0):
    ...
    :param vox: voxel object
    :param chunksize: number of rays to consider at one time (use smaller if memory issues encountered)
    :param initial_index: where to begin
    :return: rsgm
    """
