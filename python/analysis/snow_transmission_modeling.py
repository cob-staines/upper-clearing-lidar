...
corcoef_e = np.full((imsize, imsize), np.nan)
corcoef_s = np.full((imsize, imsize), np.nan)
pval_e = np.full((imsize, imsize), np.nan)
pval_s = np.full((imsize, imsize), np.nan)
for ii in range(0, imsize):
    for jj in range(0, imsize):
        if imrange[jj, ii]:
            ...
            corcoef_s[jj, ii], pval_s[jj, ii] = spearmanr(covariant[valid_all], np.exp(-intnum * imstack[jj, ii, valid_all]))
...
# colormap parameters
set = corcoef_s  # this is what we are plotting
...
ax.set_rmax(90)
ax.set_rgrids(np.linspace(0, 90, 7), labels=['', '15$^\circ$', '30$^\circ$', '45$^\circ$', '60$^\circ$', '75$^\circ$', '90$^\circ$'], angle=45)
...
cbar_ax.set_ylabel(r"Spearman's correlation coefficient $\rho_s$")

fig.savefig(plot_out_dir + "spearman_footprint_corcoef_" + date + "_" + canopy + "_mu_" + str(intnum) + ".png")
...
#create axes in the background to show cartesian image
ax0 = fig.add_subplot(111)
im = ax0.imshow(pval_s, cmap="Greys", clim=(0, .05))
ax0.axis("off")
...
cbar_ax = fig.add_axes([0.85, 0.20, 0.03, 0.6])
fig.colorbar(im, cax=cbar_ax)
cbar_ax.set_ylabel("P-value for Spearman's correlation coefficient")

fig.savefig(plot_out_dir + "spearman_footprint_pvalue_" + date + "_" + canopy + "_mu_" + str(intnum) + ".png")
...
fig.savefig(plot_out_dir + "spearman_footprint_corcoef_" + date + "_" + canopy + "_mu_" + str(intnum) + "_contours.png")
...
CS = ax0.contourf(pval_s, [0.05, 1], colors=[(0, 0, 0, 0.2)])
CS = ax0.contour(pval_s, [0.05], colors="k")
# plt.clabel(CS, inline=1, fontsize=8)
# not_sig = pval_e > 0.05
# not_sig[not_sig == 0] = np.nan
# im = ax0.imshow(not_sig, cmap="Greys", clim=[0, 1])

fig.savefig(plot_out_dir + "spearman_footprint_corcoef_pvalue" + date + "_" + canopy + "_mu_" + str(intnum) + "_contours.png")


# locate peak coords and values
np.nanmax(corcoef_s[phi <= 75 * np.pi / 180])
max_xy = np.where(corcoef_s == np.nanmax(corcoef_s[phi <= 75 * np.pi / 180]))
phi[max_xy] * 180 / np.pi
theta[max_xy] * 180 / np.pi

np.nanmax(corcoef_s)
max_xy = np.where(corcoef_s == np.nanmax(corcoef_s))
phi[max_xy] * 180 / np.pi
theta[max_xy] * 180 / np.pi

np.nanmin(corcoef_s)
min_xy = np.where(corcoef_s == np.nanmin(corcoef_s))
