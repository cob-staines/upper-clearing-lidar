...
rsm_in = "C:/Users/Cob/index/educational/usask/research/masters/data/lidar/synthetic_hemis/batches/lrs_hemi_optimization_100/outputs/contact_number_optimization.csv"
...
ggplot(df, aes(x=contactnum, y=rsm_med, color=ring_number)) +
  geom_point()

# remove 5th ring due to horizon clipping

df_anal = df[df$ring_number != 5,]
rsm_med_lm = lm(df_anal$contactnum ~ 0 + df_anal$rsm_med)
summary(rsm_med_lm)


fo = paste0("hat(y) == ", sprintf("%.5f",rsm_med_lm$coefficients['df_anal$rsm_med']), " * x")



ggplot(df_anal, aes(x=rsm_med, y=contactnum, color=ring_number)) +
  geom_point() +
  geom_abline(intercept = 0, slope = rsm_med_lm$coefficients['df_anal$rsm_med']) +
  annotate("text", x=60, y=.20, label=fo, parse=TRUE) +
  annotate("text", x=60, y=.15, label=r2, parse=TRUE) +
