...
# lpmf
p_lpmf = ggplot(data_hs, aes(x=hs, y=lpmf15)) +
  facet_grid(. ~ date) +
  geom_bin2d(bins = bincount) +
  xlim(xmin, xmax) +
...
# lpml
p_lpml = ggplot(data_hs, aes(x=hs, y=lpml15)) +
  facet_grid(. ~ date) +
  geom_bin2d(bins = bincount) +
  xlim(xmin, xmax) +
  scale_fill_gradient(name = "count", trans = "log", breaks=c(1, 10, 100, 1000), low=clow, high=chigh) +
...
# lpmc
p_lpmc = ggplot(data_hs, aes(x=hs, y=lpmc15)) +
  facet_grid(. ~ date) +
  geom_bin2d(bins = bincount) +
  xlim(xmin, xmax) +
  scale_fill_gradient(name = "count", trans = "log", breaks=c(1, 10, 100, 1000), low=clow, high=chigh) +
...
p_cn = ggplot(data_hs, aes(x=hs, y=contactnum_1)) +
  ...
  xlim(xmin, xmax) +
  ylim(0, 2.5) +
  labs(x='Snow depth (m)', y='Contact number (0-15\u00B0 from zenith)') +
  scale_fill_gradient(low=clow, high=chigh) +
  theme_minimal()
